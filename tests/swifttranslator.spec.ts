...
test('Pos_Fun_0002 - Conjunction "saha" and structure preserved correctly.', async ({ page }) => {
  await page.goto('https://www.swifttranslator.com/');
  await page.getByRole('textbox', { name: 'Input Your Singlish Text Here.' }).fill('mama gedhara yanavaa, haebaeyi vahina nisaa dhaenna yannee naee.');
  await page.getByText('මම ගෙදර යනවා, හැබැයි වහින නිසා දැන්න යන්නේ නැහැ.').click();
});
...
test('Pos_Fun_0005 - English words kept as they are with Sinhala grammar.', async ({ page }) => {
  await page.goto('https://www.swifttranslator.com/');
  await page.getByRole('textbox', { name: 'Input Your Singlish Text Here.' }).fill('mage phone  eka slow velaa.')
  await page.getByText('මගේ phone එක slow වෙලා.').click();
});
...
test('Pos_Fun_0009 - Numbers mixed with Singlish handled accurately.', async ({ page }) => {
  await page.goto('https://www.swifttranslator.com/');
  await page.getByRole('textbox', { name: 'Input Your Singlish Text Here.' }).fill('mata rupiyal 5100k oone.')
  await page.getByText('මට රුපියල් 5100ක් ඕනෙ.').click();
});
...
test('Pos_Fun_0011 - Numbers mixed with Singlish handled accurately.', async ({ page }) => {
  await page.goto('https://www.swifttranslator.com/');
  await page.getByRole('textbox', { name: 'Input Your Singlish Text Here.' }).fill('api dhan inne  indea wl.')
  await page.getByText('අපි දැන් ඉන්නෙ ඉන්දියා වල.').click();
});

test('Pos_Fun_0012 - "If" condition (enavanam) converted correctly.', async ({ page }) => {
  await page.goto('https://www.swifttranslator.com/');
  await page.getByRole('textbox', { name: 'Input Your Singlish Text Here.' }).fill('oyaa padam kroth mm dennm.')
  await page.getByText('ඔයා පාඩම් කරොත් මම දෙන්නම්').click();
});

test('Pos_Fun_0013 - Negation "naehae" handled accurately.', async ({ page }) => {
  await page.goto('https://www.swifttranslator.com/');
  await page.getByRole('textbox', { name: 'Input Your Singlish Text Here.' }).fill('mama kan na,')
  await page.getByText('මම කන් නැ').click();
});

test('Pos_Fun_0014 - Plural subject "lamayi" handled correctly.', async ({ page }) => {
  await page.goto('https://www.swifttranslator.com/');
  await page.getByRole('textbox', { name: 'Input Your Singlish Text Here.' }).fill('lamaayi sindu kiynw.')
  await page.getByText('ලමයි සින්දු කියනවා.').click();
});
...
test('Pos_Fun_0016 - The system correctly handles the "r" sound combined with "tha" (ර්ත).', async ({ page }) => {
  await page.goto('https://www.swifttranslator.com/');
  await page.getByRole('textbox', { name: 'Input Your Singlish Text Here.' }).fill('lassant design krnn.')
  await page.getByText('ලස්සනට ඩිසයින් කරන්න').click();
});
...
test('Pos_Fun_0018 - Greetings and special characters handled correctly.', async ({ page }) => {
  await page.goto('https://www.swifttranslator.com/');
  await page.getByRole('textbox', { name: 'Input Your Singlish Text Here.' }).fill('subha upandinayak wewa!')
  await page.getByText('සුබ උපන්දිනයක් වෙවා!').click();
});

test('Pos_Fun_0019 - Repeated words for emphasis handled correctly.', async ({ page }) => {
  await page.goto('https://www.swifttranslator.com/');
  await page.getByRole('textbox', { name: 'Input Your Singlish Text Here.' }).fill('hari hari mama padam krnnm')
  await page.getByText('හරි හරි මම පාඩම් කරන්නම්').click();
});

test('Pos_Fun_0020 - Common symbols like @ preserved correctly.', async ({ page }) => {
  await page.goto('https://www.swifttranslator.com/');
  await page.getByRole('textbox', { name: 'Input Your Singlish Text Here.' }).fill('mama !school inne.')
  await page.getByText('මම @shool ඉන්නේ.').click();
});


test('Pos_Fun_0021 - Local food names converted accurately.', async ({ page }) => {
  await page.goto('https://www.swifttranslator.com/');
  await page.getByRole('textbox', { name: 'Input Your Singlish Text Here.' }).fill('rice ekak dhenna.')
  await page.getByText('රයිස් එකක් දෙන්න').click();
});

test('Pos_Fun_0022 - Family relation terms converted correctly.', async ({ page }) => {
  await page.goto('https://www.swifttranslator.com/');
  await page.getByRole('textbox', { name: 'Input Your Singlish Text Here.' }).fill('magee amma dhaen gedhara.')
  await page.getByText('මගෙ අම්මා දැන් ගෙදර.').click();
});

test('Pos_Fun_0023 - Shopping related inquiry handled well.', async ({ page }) => {
  await page.goto('https://www.swifttranslator.com/');
  await page.getByRole('textbox', { name: 'Input Your Singlish Text Here.' }).fill('me cap ek kiyd.')
  await page.getByText('මෙ cap එක කියද').click();
});
